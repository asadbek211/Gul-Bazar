...
class FlowerDetailsFragment : Fragment(R.layout.fragment_flower_details) {
    ...
    private val binding by viewBinding { FragmentFlowerDetailsBinding.bind(it) }
            ...
            if (!networkCheck(requireContext())){
                ...
                message.setTitle("Gul Bazar")
                    ...
                    .setPositiveButton("Ha") { message, _ ->
                        flowerData.id?.let { it1 -> flowerDetailsViewModel.deleteAnnounceById(it1) }
...
//            val navController =
//                Navigation.findNavController(requireActivity(), R.id.mainContainer)
//            navController.navigate(R.id.action_flowerDetails_to_editFlower, bundle)
//        }
    ...
    private fun deleteAnnounceObserve() {
        flowerDetailsViewModel.deleteAnnounceResult.observe(viewLifecycleOwner, Observer {
            when (it.status) {
                ResourceState.SUCCESS -> {
